...
\begin{document}
	...
	\section{Approach and Methodology}
		...
		\subsection{Comparison criteria}
			The goal of the thesis is to discuss the advantages and drawbacks of the two proposed methods of adding automated tests to the OMS system. The selection of the comparison criteria is made based on the background study performed on automated testing. 

			\subsubsection{Development time}
			Time is an obvious factor to take into consideration when comparing the two methods. Time is a key constraint in software development and it is necessary to know how costly the two methods are for the development team. 

			During the short time span available for the thesis, it is not possible to get enough data in to do a statistical analysis on but it can give hints towards possible costs.

			\paragraph{Time which is recorded}			
			
			Time is recorded in two parts: the development time for creating the tests and the time needed to update them after the change has been introduced. 

			Development time gives an indication of the initial effort needed to introduce testing. 

			The time required for updating on the other hand is related to the maintainance effort of keeping the automated tests functional when the system changes. 

			The time that is recorded is the time spent working on the problem at hand. 
		
			\paragraph{Time which is not recorded}
			
			Time used for example for discussions and solving problems with the test or development environment will not be recorded as efficient time spent on the test or refactoring effort.
...
End Sub
	...
	\section{Results}
		...
		In the tables below we can see the times for developing the test cases before the anticipated change is rolled on (base scenario) and after it has been introduced (updated scenario).
			...
			Updating the GUI tests takes a similar amount of time in both cases but it is not possible to tell if this is a coincidence or if it has any meaning. 
		...
		\subsection{Changes required}
			...
			In the table below, the number of GUI controls accessed by the GUI tests are detailed as well as the difference after the change was introduced to the system. 

			\begin{table}[h!]
				\begin{tabular}{l l l l}
				        & Base scenario & Updated scenario & Delta\\
				 Service Object Case  & 5      & 7     & +2 \\
				 Odometer Case & 12     & 14    & +2 \\
				 \end{tabular}
				 \caption{Amount of GUI controls used in the tests}
			 \end{table}

			GUI tests are often called fragile because changes in the GUI will break the tests. In both the Service Object Case and the Odometer Case, two components have to be added to make the new test run. 

			In the Service Object Case, the test would break when the new scenario was implemented. In the Odometer Case, the test didn't break but it was necessary to add a new scenario to capture the new logic. 

			A lot of the components are only used in the steps leading up the form where the business logic is tested. 

			A change of this size does not seem to affect a lot of GUI component.  

			It is possible to conclude that the changes that have been implemented here are relatively small and comparable in size. We know from the previous analysis of the work items for the two modules that these types of changes are typical for the Workshop and Fuel modules. It seems that this could be a typical amount of change needed to update GUI tests in these modules.
		...
		\subsection{Test runtime}
			...
			\subsubsection{Data}
			Using the testrunner in Visual Studio, data on the time it takes to run the tests has been gathered. 
			...
			Surprisingly, the second case utilizes more components (see 8.2 Changes required) yet runs faster. An explanation for this is that when White searches the GUI for a specific component it goes through all of the components in the target window before returning. This means that search will be slower in a window with many components than in one with fewer components. This is the case here as the window containing the list of jobs in the service object list case is more complex than the fuel outtake window in the other case.

			The difference between the GUI tests for the Service Object Case and the Odometer Case is six fold which means that the runtime for GUI tests can vary a lot. 

			It is possible to conclude that the amount of components utilized in a test is not a indicator of how long it will take to run the test. On the other hand, the complexity of those components or the amount of components on the view might. 
			...
			IS THERE A CONNECTION HERE TO UNIT TEST PRACTICES?

			\paragraph{Differences between the two test types}			
			There is a magnitude of difference in the run time for the different test types. The GUI tests need to launch the application and navigate through it before the assertion can be made whereas the unit tests simply test a unit of functionality.

			In order to test a lot of different inputs with GUI it would take some time but this can be achieved swiftly with the unit tests.
		...
		\subsection{Lessons learned}
			...
			WHEN WILL THE TESTS RUN? DIFF OCCASIONS! Because run time and other stuff


		\subsection{Threats to validity}
		Some aspects which might influence the results. 

		\subsubsection{Construction of the test cases}
		Since one of the first steps is selecting case studies based on which change is implemented in the system, the nature of the change is known. This is likely to influence how the test cases are designed and thus the amount of change required to update the tests so that they fit the new scenario. 

		This risk is impossible to mitigate given the procedure used. The change which is about to be applied is known at the time of the test implementation and it is impossible to completely disregard that information.
